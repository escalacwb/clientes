  ...
  resultados as (
    ...
    left join lateral (
      ...
      where pa.status = 'finalizado'
        ...
        and coalesce(pa.fim_execucao, pa.inicio_execucao)::date <= a.data_revisao_proativa + params.janela_dias
      order by coalesce(pa.fim_execucao, pa.inicio_execucao) asc
      limit 1
    ) retorno on true
  )
  select *
